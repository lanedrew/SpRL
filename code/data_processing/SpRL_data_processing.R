# Depends on the "crown_attributes_2015.csv" file in the google drive
...
for(i in 1:20){
  ...
  for(j in 1:20){
    ...
    pp.list.i[[j]] <- ppp(pp.data$XTOP, pp.data$YTOP, c(pp.15.grid$xgrid[i], pp.15.grid$xgrid[i+1]), c(pp.15.grid$ygrid[j], pp.15.grid$ygrid[j+1]), marks = pp.data$CANVOL2015)
  }
...
low.dens.exp.pp <- ppp(low.dens.exp.data$XTOP, low.dens.exp.data$YTOP, c(low.win.exp[1], low.win.exp[2]), c(low.win.exp[3], low.win.exp[4]), marks = low.dens.exp.data$CANVOL2015)
low.times <- list()
...
results_file <- paste0("low_dens_data_exp.csv")
write_csv(low.dens, file = results_file)
#write_csv(low.dens, file = "low_dens_sizes_exp.csv")


## medium density expanded area
...
med.dens.exp.pp <- ppp(med.dens.exp.data$XTOP, med.dens.exp.data$YTOP, c(med.win.exp[1], med.win.exp[2]), c(med.win.exp[3], med.win.exp[4]), marks = med.dens.exp.data$CANVOL2015)
...
results_file <- paste0("med_dens_data_exp.csv")
write_csv(med.dens, file = results_file)
#write_csv(as.data.frame(med.dens$size), file = "med_dens_sizes_exp.csv")


## high density expanded area
...
high.dens.exp.pp <- ppp(high.dens.exp.data$XTOP, high.dens.exp.data$YTOP, c(high.win.exp[1], high.win.exp[2]), c(high.win.exp[3], high.win.exp[4]), marks = high.dens.exp.data$CANVOL2015)
high.times <- list()
...
results_file <- paste0("high_dens_data_exp.csv")
write_csv(high.dens, file = results_file)
#write_csv(as.data.frame(high.dens$size), file = "high_dens_sizes_exp.csv")
