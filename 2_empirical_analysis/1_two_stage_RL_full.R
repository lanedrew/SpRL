######################################################################################
#### This script runs the two stage linkage model sampler for the empirical data. ####
######################################################################################
...
seed_num <- 90210 + as.numeric(chain) - 1
set.seed(seed_num)
...
## Run the model
linkage_post <- run_mcmc_SpRL_linkage_arma_ft(n_iter = 2500, Y_mat = Y_mat, N = N, m = c(file_size),
...
res_rds <- paste0("./2_empirical_analysis/model_results/record_linkage_model/empirical_linkage_full_results_N_",
                  N_threshold, "_sigma2_prior_", sigma2_prior, "_chain_", chain,".rds")
